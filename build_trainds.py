...
def build_paths(x0: NDArray, x1: NDArray, n_steps: int) -> NDArray:
    t = np.linspace(EPSILON, 1, num = n_steps).reshape(-1, 1, 1)   # (n_t, 1, 1)
...
def build_trainds(x1: NDArray, n_steps: int) -> tuple[NDArray, NDArray]:
    ...
    xt = build_paths(x0, x1, n_steps)   # Paths from noise to data
    
    vt = x1 - x0                        # Constant velocity along the paths
    yt = np.repeat(vt[None], n_steps, axis = 0)
